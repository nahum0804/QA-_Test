...
test('TC-PROJ-45 Crear proyecto con descripción opcional (si está disponible)', async () => {
  // 1. Login
  ...
  // 4. Llenar el nombre del proyecto
  const projectName = `QA Proyecto Desc Opcional ${Date.now()}`;
  ...
  // 5. (Opcional) Llenar la descripción SOLO si el campo existe
  let descInput;
  try {
    // Ajusta este selector si sabes el id/name real del campo descripción
    descInput = await driver.findElement(
      By.css('textarea[name="description"]')
      // o: By.id('project-description')
      // o: By.xpath('XPATH_DEL_CAMPO_DESCRIPCION')
    );
  } catch {
    // Si no existe el campo descripción, no hacemos nada (es opcional)
  }

  if (descInput) {
    await descInput.sendKeys('Descripción opcional agregada desde la TC-PROJ-45');
  }
  ...
  // 7. Validar que el proyecto aparece en el listado
