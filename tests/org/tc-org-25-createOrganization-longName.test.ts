...
test("TC-ORG-25: Crear organización con nombre demasiado largo (>100 caracteres)", async () => {
    const longName = "A".repeat(120);

    await driver.findElement(By.xpath("//button[contains(., 'Create Organization')]")).click();

    await driver.findElement(By.id("organizationName")).sendKeys(longName);
    ...
    expect(err).toMatch(/too long|length/i);
});
