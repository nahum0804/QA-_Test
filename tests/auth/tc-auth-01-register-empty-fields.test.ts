...
test('TC-AUTH-01 Registro con todos los campos vacíos muestra mensajes de error', async () => {
  ...
  const error3 = await driver.findElement(By.xpath(error3XPath));
  
  
  const errorsAreVisible = (await error1.isDisplayed()) && (await error2.isDisplayed()) && (await error3.isDisplayed());
