...
test('TC-AUTH-18 Login con contraseña inválida debe mostrar mensaje dentro del modal (span dentro del div)', async () => {
  ...
  await passwordInput.sendKeys("PasswordTotalmenteIncorrecta123!");
...
`TC-AUTH-18 – Caso NO definido en la app:
  • Se intentó login con contraseña inválida para un usuario existente.
...
`TC-AUTH-16 – Caso NO definido en la app:
  ...
  • La app no está mostrando el mensaje esperado para contraseña inválida.`
    );
    return;
  }
